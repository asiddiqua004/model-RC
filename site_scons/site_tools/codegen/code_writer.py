...
class CodeWriter(object):
    def __init__(self, dbc_filepath, dbc_node_name):
        ...
        if (self._dbc_node_name != GENERATE_ALL_NODE_NAME) and (
            self._dbc_node_name not in self._valid_node_names
        ):
            raise InvalidDBCNodeError(
                "Invalid node [{}]! Available nodes {}".format(
                    self._dbc_node_name, self._valid_node_names
                )
            )
    ...
    def _mia_user_dependencies(self):
        ...
        for message in self._dbc.messages:
            self._stream.write(
                "extern const uint32_t dbc_mia_threshold_{0};\n".format(message.name)
            )
        ...
        for message in self._dbc.messages:
            suggested_mia_time = (
                ...
                else (" // Suggested MIA threshold: (3*{0})".format(message.cycle_time))
            )
            self._stream.write(
                ("extern const dbc_{0}_s ".format(message.name)).ljust(40)
            )
            self._stream.write(
                "dbc_mia_replacement_{0};{1}\n".format(message.name, suggested_mia_time)
            )
    ...
    def _message_header_instances(self):
        ...
        for message in self._dbc.messages:
            self._stream.write(
                "static const dbc_message_header_t dbc_header_{0}".format(
                    message.name
                ).ljust(80)
            )
            self._stream.write(
    ...
    def _get_decode_signal_code(self, signal, raw_sig_name="raw"):
        ...
        while remaining > 0:
            ...
            s = ""
            s += "  {0} |= ((uint64_t)((bytes[{1}] >> {2})".format(
                raw_sig_name, byte_num, (bit_pos % 8)
            )
            s += " & 0x{0})) << {1}".format(
                format(2 ** bits_in_this_byte - 1, "02x"), bit_count
            )
        ...
        unsigned_code = "message->{0} = {1}(({2} * {3}f) + ({4}));\n".format(
            signal.name, enum_cast, raw_sig_name, signal.scale, signal.offset
        )
    ...
    def _get_encode_signal_code(self, signal, raw_sig_name):
        ...
        if signal.minimum is None and signal.maximum is None:
            ...
            min_max_comment = " within range of [{0} -> {1}]".format(
...
>>>>>>> creating new branch for driver_node
        ...
        if signal.is_signed:
            signal_comment = "  // Encode to raw {0}-bit SIGNED signal with scale={1}{2}{3}\n".format(
                signal.length, signal.scale, offset_string, min_max_comment
            )
        else:
        ...
        while remaining > 0:
            ...
            s = ""
            s += "  bytes[{0}] |= (((uint8_t)({1} >> {2})".format(
                byte_num, raw_sig_name, str(bit_pos - signal.start).rjust(2)
            )
            s += " & 0x{0}) << {1})".format(
                format(2 ** bits_in_this_byte - 1, "02x"), str(bit_pos % 8)
            )
    ...
    def _mia_methods(self):
        ...
        for message in self._dbc.messages:
            self._stream.write(
                "static inline bool dbc_service_mia_{0}(dbc_{0}_s *message, const uint32_t increment_mia_by) {{\n"
        ...
        if not message.is_multiplexed():
            for signal in message.signals:
                type_and_name = "{0} {1}".format(
                    self._get_signal_type(signal), signal.name
                )
    ...
    def _message_is_relevant(self, message):
        return (message.senders[0].upper() == self._dbc_node_name.upper()) or (
            GENERATE_ALL_NODE_NAME == self._dbc_node_name.upper()
        )
