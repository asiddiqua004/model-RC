...
static dbc_BRIDGE_GPS_HEADINGS_s can_msg_bridge_gps_headings;
...
void can_bus_message_handler__manage_mia_10Hz(void) {
  ...
  if (dbc_service_mia_BRIDGE_GPS_HEADINGS(&can_msg_bridge_gps_headings, mia_increment_value)) {
    printf("Bridge Node MIA\n");
    gpio__reset(board_io__get_led0());
    geological__update_destination_coordinates(&can_msg_bridge_gps_headings);
  }
}

void can_bus_message_handler__transmit_messages_10Hz(void) { fake_bridge__transmit_gps_messages_10Hz(); }
...
void can_bus_message_handler__handle_all_incoming_messages_10Hz(void) {
  ...
  while (can__rx(can1, &incoming_message, 0U)) {
    ...
    if (dbc_decode_BRIDGE_GPS_HEADINGS(&can_msg_bridge_gps_headings, header, incoming_message.data.bytes)) {
      geological__update_destination_coordinates(&can_msg_bridge_gps_headings);
